...
4. 不臆断但不漏报:仅在能用“受审文本原文 vs 所提供的法条原文”直接对照时才下结论;需要清单外法规才能判断的,写入 suggested_searches。注意:“遗漏(omission)”本身是有外规支撑的(外规要求 X、受审文本缺 X),受审文本全文已完整提供给你,应主动核查并报告,不得因谨慎而漏报遗漏类问题。
...
【缺口处理】若判断某受审文本条款需要检索结果之外的法规才能审查,不要猜法条,而是在 suggested_searches 中给出建议检索关键词,供用户补搜后重审。
...
【输出】严格输出单个 JSON 对象,不要输出 JSON 之外的任何文字、解释或 Markdown 代码块标记。JSON 结构:
{
  ...
  "suggested_searches": [ { "trigger_clause_no": "<受审文本条款>", "keywords": ["<词>"], "rationale": "<为什么需要补搜>" } ],
